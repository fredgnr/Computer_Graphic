...
void CComputer_GraphicDoc::Bezier(CClientDC *pDc, CPoint *pointgroup, int Point_num)
{
	int n = 0;
	
	while (n < Point_num - 1) {
		CPoint temp = pointgroup[n];
		//点数满足三次曲线的部分
		if (n+3 <= Point_num-1) {
			float t1, t2, t3, t4;
			float dt = 1.0 / 10;//参数t的间隔，分10段，即用10段直线表示一段曲线pDC->MoveTo(p1);//移到起点
			for (int i = 1; i <= 10; i++)//用Bezier参数方程计算曲线上等间隔的10个点
			{
				...
				CPoint p;
				p.x = (int)(t1*float(pointgroup[n].x) + 3 * t2*float(pointgroup[n + 1].x) + 3 * t3*float(pointgroup[n + 2].x) + t4*float(pointgroup[n + 3].x));
				p.y = (int)(t1*float(pointgroup[n].y) + 3 * float(t2*pointgroup[n + 1].y) + 3 * t3*float(pointgroup[n + 2].y) + t4*float(pointgroup[n + 3].y));
				pDc->MoveTo(temp);
				pDc->LineTo(p);
				temp = p;
		...
		else if (n + 2 == Point_num - 1) {
			float t1, t2, t3;
			float dt = 1.0 / 10;//参数t的间隔，分10段，即用10段直线表示一段曲线pDC->MoveTo(p1);//移到起点
			for (int i = 1; i <= 10; i++)//用Bezier参数方程计算曲线上等间隔的10个点
			{
				t1 = (1.0 - i*dt)*(1.0 - i*dt);	//计算(1-t)3
				t2 = 2 * i*dt*(1.0 - i*dt);	//计算t(1-t)2
				t3 = i*dt*i*dt;	//计算t3
				CPoint p;
				p.x = (int)(t1*float(pointgroup[n].x) +  t2*float(pointgroup[n + 1].x) + t3*float(pointgroup[n + 2].x) );
				p.y = (int)(t1*float(pointgroup[n].y) + t2*float(pointgroup[n + 1].y) +t3*float(pointgroup[n + 2].y));
				pDc->MoveTo(temp);
				pDc->LineTo(p);
				temp = p;
			}
			...
			pDc->MoveTo(temp);
			pDc->LineTo(pointgroup[Point_num - 1]);
			break;
		}
	}
}

void CComputer_GraphicDoc::Generate_Graph(CClientDC *pDC)
{
	group[0].x = 100; group[0].y = 100;//图形数据准备group[1].x=200;group[1].y=100;
	group[1].x = 100; group[1].y = 200;
	group[2].x = 200; group[2].y = 200;
	group[3].x = 200; group[3].y = 100;
...
void CComputer_GraphicDoc::Draw_Graph(CClientDC *pDC) 
{
	for (int i = 0; i < group_num-1; i++) {
		pDC->MoveTo(group[i]);
		pDC->LineTo(group[i + 1]);
	}
}